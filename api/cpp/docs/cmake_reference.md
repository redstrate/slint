...
**NOTE:** Be careful when calling slint_target_sources multiple times. This creates conflicts between namespaces. So for example, if you call slint_target_sources twice on the same target and they are both unnamespaced, generated symbols like `SharedGlobals` may clash. 
